...
function renderCard(item) {
  ...
  return `
    <div class="skill-card flex flex-col justify-between p-6 sm:p-7 space-y-6">
      ...
      <!-- Card Action Footer -->
      <div class="mt-4 flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
        <a href="${item.url}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-bold bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 transition-colors">
        ...
        <button data-copy="${escapeHtml(item.action_content)}" class="btn-primary inline-flex items-center gap-1.5 px-5 py-2 rounded-full text-xs font-bold transition-all shadow-sm cursor-pointer">
