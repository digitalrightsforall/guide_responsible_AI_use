...
// Helper to generate compact category matrix table
function generateCategoryMatrix(cat, items, isZh = false) {
  ...
  catItems.forEach((item, idx) => {
    const timingShort = isZh
      ? { 'pre-input': '⏳ 输入前', 'during-chat': '💬 交互中', 'pre-handoff': '📤 交付前', 'post-session': '🧹 归档时' }[item.timing] || item.timing
      : { 'pre-input': '⏳ Pre-Input', 'during-chat': '💬 During-Chat', 'pre-handoff': '📤 Pre-Handoff', 'post-session': '🧹 Post-Session' }[item.timing] || item.timing;

    const isSetup = (item.status && (item.status.stage_en || item.status.stage || '').includes('Requires')) ||
                    (item.status && (item.status.stage_cn || item.status.stage || '').includes('需配置'));
    const statusShort = isSetup
      ? (isZh ? '🟡 需依赖' : '🟡 Setup')
      : (isZh ? '🟢 开箱即用' : '🟢 Ready');

    const stdName = item.standard_alignment
      ? (isZh ? item.standard_alignment.name_zh : item.standard_alignment.name_en)
      : (isZh ? '社区开源规范' : 'Community Standard');
...
function generateReadmeEn() {
  ...
  md += `* [🗂️ Repository Structure](#️-repository-structure)\n`;
  md += `* [🧪 Local Development & Quality Gates](#-local-development--quality-gates)\n`;
  ...
  md += `│   ├── data/\n`;
  md += `│   │   └── items.json          # Master database of all 43 vetted skills\n`;
  ...
  md += `npm test\n\n`;
  md += `# 2. Test live HTTP 200 reachability for all 43 remote GitHub repositories\n`;
...
function generateReadmeCn() {
  ...
  md += `1. 📖 **[实务长篇指南 (Field Guide)](https://putongren.org/ai-skills/)**（*网站首页 / \`site/index.html\`*）：\n`;
  md += `   深度叙事长篇指南。系统解构算法时代的平民处境（数据主权剥夺、自信幻觉与认知依附），解析为何宏大伦理必须落地为桌面防线，并通过 **5 大核心工作流场景** 穿针引线串联全量 43 项技能，提供 112 处微卡片原地交互与 3 大角色一日安全工作流。\n\n`;
  md += `2. 🛡️ **[实务技能库 (Skill Library)](https://putongren.org/ai-skills/library.html)**（*交互技能库 / \`site/library.html\`*）：\n`;
  md += `   开箱即用的交互式技能筛选与搜索平台。支持按 4 大实务分类、4 大工作流时序进行即时组合过滤，提供实时关键词搜索、运行状态与实证弹窗、技术标准溯源与一键复制安装命令。\n\n`;
  md += `3. 💻 **[GitHub 开源主仓库](https://github.com)**（*本仓库*）：\n`;
  ...
  md += `  \`\`\`\n\n`;
  md += `> [!IMPORTANT]\n`;
  md += `> **必须完整复制整个技能文件夹**：许多高质量技能内置了独立的 Python/Bash 校验脚本、威胁规则库与参考模板。绝不要只单份复制 \`SKILL.md\`，请始终按各条目的安装命令完整拉取整个目录。\n\n`;
  md += `---\n\n`;
  ...
  md += `│   ├── data/\n`;
  md += `│   │   └── items.json          # 全量 43 项技能核心主数据库\n`;
  md += `│   └── sitemap.xml             # 搜索引擎索引地图\n`;
  md += `├── scripts/                    # 自动化与质检工具\n`;
  md += `│   ├── verify-data.js          # Schema 格式与数据门禁测试 (npm test)\n`;
  md += `│   ├── verify-remote.js        # 43 个 GitHub 链接实时存活巡检 (npm run test:remote)\n`;
  ...
  md += `npm test\n\n`;
  md += `# 2. 批量并发探测 43 项技能的 GitHub 真实在线状态 (HTTP 200 巡检)\n`;
  md += `npm run test:remote\n\n`;
